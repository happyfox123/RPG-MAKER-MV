 ...
 *=============================================================================*/
/*:
 * @plugindesc 這個插件會在你按下某個鍵時，將整個地圖的圖片儲存到 "Mapshots" 資料夾中。<OrangeMapshot>
 * @author Hudell
 *
 * @param 使用地圖名稱
 * @desc 如果為true，檔案名稱將是地圖的名稱。如果為false，則將是編號。
 * @default true
 *
 * @param 圖層類型
 * @desc 0 = 所有, 1 = upper and lower, 2 = 將所有事物分開
 * @default 0
 *
 * @param 繪製自動陰影
 * @desc 將此設置為false以禁用地圖截圖中的自動陰影。
 * @default true
 *
 * @param 繪製事件
 * @desc 將此設置為false，以停止在完整位圖上繪製事件。
 * @default true
 *
 * @param 鍵盤碼
 * @desc 將使用的按鍵代碼（44 = 截圖鍵）。
 * @default 44
 *
 * @param 圖片類型
 * @desc 應該生成哪種類型的圖片。可以是 png、jpeg 或 webp。
 * @default png
 *
 * @param 圖片品質
 * @desc 如果圖片類型是 jpeg 或 webp，你可以將此值設為 0 到 100 之間的數字，表示圖片的品質。
 * @default 70
 *
 * @param 圖片路徑
 * @desc 圖片將被儲存的路徑。
 ...
 *
 * @help
 * 你可以透過此網站檢查按鍵的 keyCode： http://www.javascriptkeycode.com
 * 這個網站提供了完整的按鍵與 keyCode 對照表，方便開發者查詢。如果你需要查詢特定按鍵的 keyCode，可以打開該網站並按下鍵盤上的按鍵，它會顯示對應的數值。
 * 例如：
 * 
 * Enter 鍵 的 keyCode 是 13
 * 
 * 空格鍵 的 keyCode 是 32
 * 
 * Print Screen 鍵 的 keyCode 是 44
 */
